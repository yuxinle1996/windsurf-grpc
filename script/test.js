...
function parseFields(fieldsStr) {
  ...
  // 匹配每个字段对象 {no: ..., name: ..., kind: ..., T: ..., opt: ..., repeated: ...}
  const fieldRegex =
    /\{\s*no:\s*(\d+)\s*,\s*name:\s*["`']([^"`']+)["`']\s*,\s*kind:\s*["`']([^"`']+)["`']\s*(?:,\s*T:\s*([^,}]+))?\s*(?:,\s*opt:\s*([^,}]+))?\s*(?:,\s*repeated:\s*([^,}]+))?\s*[^}]*\}/g;

  let fieldMatch;
  while ((fieldMatch = fieldRegex.exec(fieldsStr)) !== null) {
    const [, no, name, kind, type, opt, repeated] = fieldMatch;
    ...
    fields.push({
      ...
      optional: opt?.trim() === "!0" || opt?.trim() === "true",
    });
  }
...
function parseEnumValues(enumValuesStr) {
  ...
  return values.sort((a, b) => a.no - b.no);
}
...
function generateSingleProtoFile(messages, enums, packageName) {
  ...
  messages.forEach((message) => {
    ...
    message.fields.forEach((field) => {
      ...
      proto += `  ${repeated}${optional}${field.type} ${field.name} = ${field.number};\n`;
    });
...
function reverseEngineerProto(jsFilePath, outputDir) {
  ...
  console.log("packageGroups", packageGroups.get("codeium_common_pb"));
